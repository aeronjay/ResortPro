...
namespace ResortPro
{
    public partial class bookings : Form
    {
        ...
        {
            string sql = "SELECT ID, fullName, email, contactNumber, checkInDate, (numberAdults + numberKids) AS peopleNumber, accommodationType, paid, totalPrice, Done, Confirmed FROM bookings WHERE Done = False ORDER BY ID DESC";
                ...
                using (OleDbConnection connection = new OleDbConnection(dbOp.ConnectionString))
                {
                    using (OleDbDataAdapter adapter = new OleDbDataAdapter(sql, connection))
                    {
                        ...
                        foreach (DataRow row in dataTable.Rows)
                        {
                            ...
                            bunifuDataGridView1.Rows[rowIndex].Cells["Confirmed"].Value = row["Confirmed"];
